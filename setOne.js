...
// CODE HERE
let onelineFunc = (parameter1, parameter2) => `The first parameter is ${parameter1}. The second parameter is ${parameter2}.`



// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

// console.log(onelineFunc(4, 5))
...
let fullNameFunc = fullName => {
    console.log(`Hello, my full name is ${fullName}`)
}

//greeting("john", "doe", fullNameFunc)
...
let pricesPlusTax = (prices, callback) => {
    prices.forEach(price => {
        totalCost.push((price + (0.20 * price)))
    })
    callback(totalCost)
}
...
// CODE HERE
let multiplyingFactory = number1 =>{
    return number2 =>{
        if(number1 >= 5){
